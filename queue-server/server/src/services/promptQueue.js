...
export async function chatReplyToPrompt(id, { text, userId = null } = {}) {
  ...
  const thread = (omitted > 0 ? `(${omitted} earlier message${omitted === 1 ? '' : 's'} not shown)\n\n` : '')
    + messages
      ...
      .join('\n\n');
  const system = `Respond to the latest message from the human as part of an ongoing conversation about this task. Plain English, no jargon, no file names unless I explain them.\n\nYou can read this project's files to check what is actually true before answering — and you should, whenever the question is about how something behaves ("does this also cover X?"). Never guess at it. You can only look: you can never change anything, and you are not doing the task, just answering the question.\n\nLooking at the code does NOT change how you talk about it. Never quote a file name, a line number, a function name, an id, or a web address in your answer — not even in passing, and never as a reference at the end. You are talking TO the owner of the app: say \"you\", and never refer to them by name in the third person. Say what the thing DOES on screen, in the words the person using the app would use. You looked so that your answer is right, not so that you can show where you looked.\n\n${USER_FACING_STYLE}\n\n`;
  const context = [
