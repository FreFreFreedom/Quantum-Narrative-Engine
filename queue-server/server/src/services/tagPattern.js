...
function buildPrompt(tag, examples) {
  ...
  return [
    ...
    `\nExplain, in plain language (2-4 sentences, no jargon, not a dictionary definition), what this tag represents as a recurring pattern — the underlying dynamic or structure it names, not just a paraphrase of the tag's words. `,
    `Write for someone who has never seen this tag before and wants to understand what actually connects entities that share it.\n`,
  ...
  return async function getTagPatternExplanation(tag, { force = false } = {}) {
    ...
    try {
      resp = await fetch(ANTHROPIC_API_URL, {
        ...
        body: JSON.stringify({ model: CHAT_MODEL, max_tokens: 350, messages: [{ role: 'user', content: buildPrompt(tag, examples) }] }),
      });
    } catch (e) {
