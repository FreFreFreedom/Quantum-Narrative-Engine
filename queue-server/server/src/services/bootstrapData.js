...
const KNOWLEDGE_DESCRIPTIONS = {
  'ontology.md': 'QNE standing reference: the ontological/semantic/analogical layers, Integration Continuum, Scale Echo, and the Quantum Narrative Engine framing. The fullest statement of the paradigm.',
  ...
  'fractal_operational_core.md': 'The operational extension of ontology.md §1: what an entity is (self-maintenance, internal fragmentation), why entity and event are the same kind of thing, the three layers as three acts, the platform as a prosthetic analogical layer, mechanisms for integration and shadow, the three navigation moves (vertical/horizontal/entanglement), a grouped list of real-world projects working each layer, a reference shelf of works and why each matters to the vision (including the metaphysical lineage: the jewel-net of Indra, the implicate order, morphic resonance, the four worlds), where the field is going, the fractal reading (ontology as a generating rule; analogical strength as scales survived), a catalogue of mathematical instruments sorted by which of the three acts each serves (including sheaves for measuring where fragments fail to glue), how the instruments unlock each other, the fold (the paradigm applied to its own construction, required by the claim rather than optional), the nameless interior (a part has no identity except its relations, correspondence is shape not overlap, the engine must discover rather than score, and naming must never feed the matcher), and the division of labour: Antoine does the seeing, the agent does the formalism. Append to this file rather than re-deriving the vision in chat.',
};
