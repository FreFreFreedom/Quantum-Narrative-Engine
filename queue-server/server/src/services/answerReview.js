...
const rewritePrompt = ({ question, answer, faults, lens }) => `${lens ? `=== THE LENS ===\n${lens}\n\n` : ''}=== THE ANSWER YOU WROTE ===
${answer}
...
export async function reviewAnswer({ question, answer, lens = '', read, rewrite, onStatus = null, countWords }) {
  ...
  try {
    ...
    const r = await read(readerPrompt({ question, answer: original, words: hits }));
  ...
  try {
    ...
    const w = await rewrite(rewritePrompt({ question, answer: original, faults, lens }));
