...
| **The Narrative Mirror** — open any loop onto the human scene it costs: the deposition, the film scene, the unsent letter | Now planned: [narrative-mirror.md](narrative-mirror.md). "No new machinery" was wrong — the Room cannot see relations, loops or any civic entity, and nothing links a relation to a quotable line | Stage 4 shipped, so it is unblocked; the work is the Room's tools and saving the anatomy runs' turns. |
| **Cross-domain healing search** — retrieve a structural antidote from a domain with no shared vocabulary (the gut's sIgA buffering as a model for policing; the forest's nurse log as a model for transitional justice) | Now planned: [cross-domain-healing-search.md](cross-domain-healing-search.md). The missing non-narrative corpus is not the hard part — `shape` is four hand-written codes, which is label-matching, so the blocker is an invention | A **computed** anatomy handle derived from signed structure with no name in it. That is a research question, not a task. |
