...
|---|---|---|
| [rotate-leaked-credentials.md](rotate-leaked-credentials.md) | Housekeeping, not a feature. On 2026-08-21 the full Railway variable set was pasted into a chat **with values**, so every token and password the deployment holds is exposed in a transcript. Low likelihood of abuse (a chat, not a public paste) but outside Antoine's control. The plan also carries the audited **variable cleanup** it depends on: 14 variables the code never reads, 6 of them secrets, so deleting them *is* most of the fix and costs nothing. Rotate `RAILWAY_TOKEN` and `DATABASE_URL` first — the former can read every other variable. | **PLANNED — deferred on purpose** (Antoine, 2026-08-21) |
